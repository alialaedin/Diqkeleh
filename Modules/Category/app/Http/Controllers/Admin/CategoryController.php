...
class CategoryController extends Controller implements HasMiddleware
{
	...
	{
		return [
			new Middleware('permission:read_categories', ['index', 'show']),
	...
	{
		$category->load([
			'products' => fn(HasMany $products) => $products->orderBy('order')
		]);
	...
	public function sortProducts(Request $request, Category $category)
	{
		$request->validate(['products' => 'required|array']);
		$category->sortProducts($request);
